...
"""Outils de chargement du dataset image / annotation.

Le but de ce module est de construire un index simple reliant chaque
annotation JSON à l'image correspondante, puis de fournir des helpers
pour charger les données de manière robuste.
"""
...
class DatasetSample:
    """Association entre une image et son annotation."""
...
def _normalized_stem(path: Path) -> str:
    """Normalise le nom de fichier pour faciliter les appariements."""
...
def build_dataset_index(
    ...
    """Construit la liste des échantillons exploitables du dataset.

    Le matching se fait d'abord via le nom de fichier indiqué dans
    l'annotation. Si aucune image n'est trouvée, le code plus loin
    pourra encore utiliser l'image embarquée dans le JSON LabelMe.
    """
    ...
    for annotation_path in annotation_paths:
        ...
        if image_name:
            # On compare sur le stem pour tolérer quelques variations de chemin/extension.
    ...
    if matched_file_count == len(annotation_paths):
        warnings.append(
            f"Appariement direct reussi : {matched_file_count} annotation(s) correspondent a `dataset/images`."
        )
    elif matched_file_count == 0:
        warnings.append(
        ...
        warnings.append(
            f"{fallback_count} annotation(s) sans image correspondante dans `dataset/images`; "
...
def inspect_sample(sample: DatasetSample) -> dict:
    """Retourne toutes les informations utiles pour inspecter un échantillon."""
...
def load_sample_image(sample: DatasetSample) -> np.ndarray | None:
    """Charge l'image associée à un échantillon.

    On privilégie le fichier image réel. En cas d'échec, on retombe
    sur l'image embarquée dans l'annotation LabelMe.
    """
